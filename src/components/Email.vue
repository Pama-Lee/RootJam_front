...
export default{
    ...
    mounted(){
        ...
        mdui.snackbar({
  message: '邮件成功发送,如未收到请检查垃圾箱,重发邮件请返回上一步',
  buttonText: '上一步',
  ...
  closeOnOutsideClick: false
    ...
    methods:{
      ...
      this.$axios.post("/Register/RegisterXmu",{ email: email,token:this.token,password:pwd,t:time}).then((response) => {
...
<template>
  <div id="Login" class="mdui-row mdui-center" style="padding-top:50px">
    <div class="mdui-col-md-6 mdui-col-offset-md-3 mdui-col-sm-7 mdui-col-offset-sm-2">
        ...
        <div class="mdui-textfield-error" v-if="checkPassword">两次密码不相同</div>
        <input class="mdui-textfield-input" type="password" v-model="password" @input="changeText"/>
</div>
<div :class="textClass">
            <i class="mdui-icon material-icons">&#xe32a;</i>
        <label class="mdui-textfield-label">确认密码</label>
        <div class="mdui-textfield-error">两次密码不相同</div>
        <input class="mdui-textfield-input" type="password" v-model="cpassword" @input="changeText"/>
</div>
<div style="padding-bottom:20px"></div>
<button id="next" class="mdui-btn mdui-btn-block mdui-color-theme-accent mdui-ripple" @click="nexts">下一步</button>
</div>
    </div>
  </div>
</template>

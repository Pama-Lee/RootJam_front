...
export default {
  ...
  methods: {
    ...
    sendPayRequest: function () {
      let token = this.$cookies.get("token");
      this.$axios
        .post("/Pay/GetPay", { token: token, RM: this.money })
        .then((response) => {
          if (response.data.code == 200 && response.data.status == 1) {
...
<template>
  <div class="mdui-container">
    <div class="mdui-col-md-6 mdui-col-offset-md-3" style="padding-top: 75px">
      ...
      <h3>
        请使用电脑/平板打开此界面充值,此界面的支付二维码无法截图后在微信支付,请使用手机扫码
      </h3>
      <div id="pay">
        ...
        <input
          ...
          hidden
        />
        <button class="mdui-btn mdui-color-theme-accent mdui-ripple" disabled>
          支付宝(暂不开放)
        </button>
        <button
          class="mdui-btn mdui-color-theme mdui-ripple"
          id="submit_wechat"
          @click="wechat"
          :disabled="buttonPay"
        >
